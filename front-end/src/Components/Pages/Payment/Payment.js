...
export default function Payment() {
    ...
    let amount = tripType == "oneWay" ? parseInt(total1 ) : parseInt(total1 + total2);
    const jsonData = {
        "ticket_amount": parseInt(total1),
        "customer_name": `${passengerInfo.LastName}`,
        "customer_email": `${passengerInfo.Email}`,
        "customer_birthday": `${passengerInfo.DateOfBirth}`,
        "customer_identify": `${passengerInfo.PassportNumber}`,
        "customer_address": `${passengerInfo.City}`,
        "seat_id": `${seatId}`,
        "flight_id": parseInt(departFlight.id),
